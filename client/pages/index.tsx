...
export default function Home() {
  ...
  const createNewForm = () => {
    ...
    if (isLogin) {
      const fomrId = uuidv4();
      axios
        .put(`https://nspark.shop/form/create/${fomrId}`, { email: email })
        .then((res) => {
  ...
  return (
    <>
      ...
      <main className="mt-2">
        <div className="text-center bg-slate-600 text-white py-8 space-y-10 shadow-md">
          <div className="space-y-6 text-2xl">
            <p>
              Form Bakery helps to simplify and distribute the survey form you
              want
            </p>
      ...
      <Alert
        title={signupQuery ? "회원가입" : signoutQuery ? "계정삭제" : "로그인"}
        subTitle={
          signupQuery
            ? "회원가입에 성공하셨습니다. "
            : signoutQuery
